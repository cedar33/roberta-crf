...
class RobertaLabelingHead(nn.Module):
    """Head for sentence-level classification tasks."""

    def __init__(self, input_dim, inner_dim, num_tags, pooler_dropout, q_noise=0, qn_block_size=8):
        super().__init__()
        self.dropout = nn.Dropout(p=pooler_dropout)
        # self.out_proj = apply_quant_noise_(
        #     nn.Linear(inner_dim, num_tags), q_noise, qn_block_size
        # )
        self.dense = nn.Linear(input_dim, num_tags)
        self.crf_layer = CRF(num_tags, beam_size=5)
    ...
    def forward(self, features, **kwargs):
        x = features[:,0:-1,:]  # take <s> token (equiv. to [CLS])
        x = self.dropout(x)
        x = self.dense(x)
        targets = kwargs["kwargs"]["target"]
        loss = -1*self.crf_layer(x, targets, masks=None).sum()
        path_score, path = self.crf_layer.forward_decoder(x, masks=None)
        return loss, path_score, path


    def forward_decode(self, features, **kwargs):
        x = features  # take <s> token (equiv. to [CLS])
        x = self.dropout(x)
        x = self.dense(x)
        path_score, path = self.crf_layer.forward_decoder(x, masks=None)
